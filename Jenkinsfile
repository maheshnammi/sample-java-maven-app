  ...
  stages {
   stage ('Initialize') {
    ...
    stage('Push Artifact to S3') {
      steps {
        sh 'aws s3 cp webapp/target/webapp.war s3://jen-bucket'
      }
    }
    
   stage('Deploy to tomcat') {
     steps {
      sh 'sudo scp -i /home/ubuntu/keys/jenkins.pem -o "StrictHostKeyChecking=no" webapp/target/webapp.war ubuntu@52.66.178.185:/opt/tomcat/webapps'
     }
    }
